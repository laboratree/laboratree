...
- **pre-push** — `pytest` (backend) before code leaves your machine.

Bypass a hook once, intentionally: `git commit --no-verify` / `git push --no-verify`.

## Notes
- **API port** is `8000` (matches `frontend/.env.local` → `NEXT_PUBLIC_API_URL`). If
...
- **First run / after pulling:** install deps once — `uv sync` (repo root) and
  `cd frontend && npm install`. The scripts don't reinstall on every start.
