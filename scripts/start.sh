...
API_PORT="${API_PORT:-8000}"   # matches frontend/.env.local; override: API_PORT=8001 bash scripts/start.sh
...
( cd "$ROOT/backend" && uv run uvicorn laboratree.main:app --reload --port "$API_PORT" ) &
API_PID=$!
